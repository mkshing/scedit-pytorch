...
from scedit_pytorch import UNet2DConditionModel, save_scedit, load_scedit_into_unet
...
---
    ...
    model_card = f"""
# SDXL SCEdit DreamBooth - {repo_id}

<Gallery />

## Model description

These are {repo_id} SC-Tuner adaption weights for {base_model}.
...
def parse_args(input_args=None):
    ...
    parser.add_argument(
        "--output_dir",
        type=str,
        default="scedit-dreambooth-model",
...
def main(args):
    ...
    def save_model_hook(models, weights, output_dir):
        if accelerator.is_main_process:
            unet_sctuner_layers_to_save = None
            for model in models:
                if isinstance(model, type(accelerator.unwrap_model(unet))):
                    unet_sctuner_layers_to_save = {
                        k: v for k, v in model.state_dict().items() if "sc_tuners" in k
                    }
                else:
                ...
                weights.pop()
            save_scedit(
                save_directory=output_dir, state_dict=unet_sctuner_layers_to_save
            )
    ...
    def load_model_hook(models, input_dir):
        ...
        while len(models) > 0:
            ...
            if isinstance(model, type(accelerator.unwrap_model(unet))):
                ...
                raise ValueError(f"unexpected save model: {model.__class__}")
        load_scedit_into_unet(state_dict=input_dir, unet=unet_)
    ...
    if accelerator.is_main_process:
        accelerator.init_trackers("dreambooth-scedit-sd-xl", config=vars(args))
    ...
    if accelerator.is_main_process:
        ...
        save_scedit(
            save_directory=args.output_dir, state_dict=unet_sctuner_layers_to_save
        )
